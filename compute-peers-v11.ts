...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (entityType === 'recipient') {
      ...
      // Semantic proximity — categories that serve overlapping populations
      // e.g. emergency shelters, housing orgs, food banks, and human service
      // agencies all serve people experiencing homelessness/poverty
      ...
      // Count grants per category from purpose_text + org name keywords
      ...
      // Also extract mission categories from the ORG'S NAME
      // This catches orgs like "Downtown Emergency Service Center" where funders
      // describe grants generically but the name reveals the true mission
      const orgNameLower = source.name.toLowerCase();
      for (const [category, terms] of Object.entries(MISSION_KEYWORDS)) {
        for (const term of terms) {
          if (orgNameLower.includes(term)) {
            // Give name-based matches significant weight (equivalent to 3 grants)
      ...
      // Expand profile with RELATED categories at reduced weight
      // If source matches "emergency" + "food" + "human_services", also add
      // their related categories (like "housing") at 1/3 strength
      ...
      // Step 5: Build search terms — use terms from TOP categories (including expanded)
      const searchTermsForCategory = new Map<string, string[]>();
      const combinedText = allPurposeText.join(' ') + ' ' + orgNameLower;

      for (const [cat] of rankedCategories.slice(0, 8)) {
        const terms = MISSION_KEYWORDS[cat] || [];
        // Use terms that appear in purpose_text OR org name, plus default first 2 terms
        const matching = terms.filter(t => combinedText.includes(t));
        const searchTerms = matching.length > 0 ? matching.slice(0, 2) : terms.slice(0, 2);
      ...
      // Step 7: Score candidates using WEIGHTED OVERLAP
      // Two components:
      //   overlap = sum of source weights for matched categories (how much mission aligns)
      //   focus   = overlap / candidate breadth (penalises unfocused mega-orgs)
      // Final = 60% overlap + 40% focus — rewards both alignment and concentration
      ...
      for (const [ein, data] of candidateHits) {
        ...
        // Focus = overlap / breadth (penalises mega-orgs that match everything)
        const focus = weightedOverlap / data.categories.size;

        // Combined: overlap matters most, focus is a secondary filter
        const focusScore = weightedOverlap * 0.6 + focus * 0.4;

        // Require minimum 2 matching grants to filter noise
